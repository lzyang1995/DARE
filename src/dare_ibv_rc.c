...
if (server->next_lr_step == LR_UPDATE_END) {
    ...
    sprintf(posted_sends_str, "%s %d-log", posted_sends_str, i);
        ...
        else
        {
            ...
            fprintf(post_send_inter, "p%d\t%d\t\t%"PRIu64"\t%9.3lf\n", i, server->next_lr_step, server->cached_end_offset, HRT_GET_USEC(rdtsc_wor));
        }
#else
        if(lzyang_first == 0)
        {
                        ...
                        int *posted_sends )
{
    ...
    while(1) {
        ...
        if(!lzyang_flag)
        {
            ...
            {
                fprintf(lzyang_fp_ack, "Fail to get the start timestamp\n");
            }
        }     
        else
        {
...
//TIMER_INFO(log_fp, "[p%"PRIu8"->log(:%"PRIu64")]\n", idx, server->cached_end_offset);
                        ...
                        if(lzyang_first != 0)
                        {
                            ...
                            HRT_GET_ELAPSED_TICKS(rdtsc_start, rdtsc_end, &rdtsc_wor);
                            rdtsc_start = rdtsc_end;
                            fprintf(post_send_inter, "p%d\tACK OF C\t%"PRIu64"\t%9.3lf\n", idx, server->cached_end_offset, HRT_GET_USEC(rdtsc_wor));
                        }
#else
                        if(lzyang_first != 0)
                        {
