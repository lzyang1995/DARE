...
//info(log_fp, "### add log entry NOOP\n");
    ...
    //fprintf(log_fp, "APPENDED ENTRY: log_entry_len is %d, sizeof(dare_log_entry_t) is %d, entry->data.cmd.len is %d\n", log_entry_len(entry), sizeof(dare_log_entry_t), entry->data.cmd.len);
